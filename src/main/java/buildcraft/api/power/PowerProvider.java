		...
		if (!this.preConditions(receptor)) {
			...
			if (this.energyStored >= (float) this.minActivationEnergy) {
				if (this.latency == 0) {
					receptor.doWork();
					result = true;
				} else if (this.timeTracker.markTimeIfDelay(tile.worldObj, (long) this.latency)) {
					receptor.doWork();
					result = true;
				}
			}

			if (this.powerLoss > 0 && this.energyLossTracker.markTimeIfDelay(tile.worldObj, (long) this.powerLossRegularity)) {
			...
			for (int i = 0; i < 6; ++i) {
				if (this.powerSources[i] > 0) {
					int var10002 = this.powerSources[i]--;
				}
			}
